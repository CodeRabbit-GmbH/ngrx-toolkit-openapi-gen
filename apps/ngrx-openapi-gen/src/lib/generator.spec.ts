...
describe('Generator', () => {
  ...
  describe('generate', () => {
    ...
    it('generates entity models for Flight domain', () => {
      const flightModel = files.find(f => f.path === 'flight-api/flight/entities/flight.model.ts');
      ...
      expect(flightModel?.content).toContain("status: 'scheduled' | 'delayed';");
    });

    it('generates entity models for Booking domain', () => {
      const bookingModel = files.find(f => f.path === 'flight-api/booking/entities/booking.model.ts');
    ...
    it('generates store for Flight domain', () => {
      const flightStore = files.find(f => f.path === 'flight-api/flight/application/flight.store.ts');
      expect(flightStore).toBeDefined();
      expect(flightStore?.content).toContain('export const FlightStore = signalStore(');
    });

    it('generates store for Booking domain', () => {
      const bookingStore = files.find(f => f.path === 'flight-api/booking/application/booking.store.ts');
      expect(bookingStore).toBeDefined();
      expect(bookingStore?.content).toContain('export const BookingStore = signalStore(');
    });
    ...
    it('flight store imports from ngrx-toolkit', () => {
      const flightStore = files.find(f => f.path === 'flight-api/flight/application/flight.store.ts');
      expect(flightStore?.content).toContain("from '@angular-architects/ngrx-toolkit'");
    });
...
describe('Generator with primitive array fixture', () => {
  ...
  it('generates store for Airport domain with primitive string array', () => {
    ...
    expect(airportStore?.content).toContain('export const AirportStore = signalStore(');
  });
  ...
  it('generates store for Category domain with primitive number array', () => {
    const categoryStore = files.find(f => f.path === 'primitive-api/category/application/category.store.ts');
