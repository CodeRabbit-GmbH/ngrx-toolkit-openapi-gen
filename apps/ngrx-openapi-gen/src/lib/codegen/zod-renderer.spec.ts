...
describe('ZodRenderer', () => {
  ...
  describe('nullable types', () => {
    it('renders nullable string', () => {
      expect(renderer.render({ type: 'string', nullable: true })).toBe('z.string().nullable()');
    });

    it('renders nullable number', () => {
      expect(renderer.render({ type: 'number', nullable: true })).toBe('z.number().nullable()');
    });
  });

  describe('enum types', () => {
    it('renders string enum', () => {
      expect(renderer.render({ type: 'string', enum: ['pending', 'completed'] }))
        .toBe("z.enum(['pending', 'completed'])");
    });

    it('renders nullable enum', () => {
      expect(renderer.render({ type: 'string', enum: ['active', 'inactive'], nullable: true }))
        .toBe("z.enum(['active', 'inactive']).nullable()");
    });

    it('renders single value enum as enum with single value', () => {
      expect(renderer.render({ type: 'string', enum: ['only'] }))
        .toBe("z.enum(['only'])");
    });

    it('escapes special characters in enum values', () => {
      expect(renderer.render({ type: 'string', enum: ["it's", "value"] }))
        .toBe("z.enum(['it\\'s', 'value'])");
    });
  });

  describe('array types', () => {
    it('renders array of strings', () => {
      expect(renderer.render({ type: 'array', items: { type: 'string' } }))
        .toBe('z.array(z.string())');
    });

    it('renders array of numbers', () => {
      expect(renderer.render({ type: 'array', items: { type: 'number' } }))
        .toBe('z.array(z.number())');
    });

    it('renders array with explicit unknown items as unknown array', () => {
      expect(renderer.render({ type: 'array', items: {} }))
        .toBe('z.array(z.unknown())');
    });

    it('renders nullable array', () => {
      expect(renderer.render({ type: 'array', items: { type: 'string' }, nullable: true }))
  ...
  describe('object types', () => {
    ...
    it('renders empty object as record', () => {
      expect(renderer.render({ type: 'object' }))
        .toBe('z.record(z.string(), z.unknown())');
    });
  ...
  describe('reference types', () => {
    it('renders $ref as lazy schema reference', () => {
      expect(renderer.render({ $ref: '#/components/schemas/Task' }))
        .toBe('z.lazy(() => TaskModelSchema)');
    });

    it('renders $ref with custom suffix', () => {
      const customRenderer = new ZodRenderer({ modelSuffix: 'Entity' });
      expect(customRenderer.render({ $ref: '#/components/schemas/User' }))
        .toBe('z.lazy(() => UserEntitySchema)');
    });
  });
  ...
  describe('combination types', () => {
    ...
    it('renders allOf as intersection', () => {
      ...
      })).toBe('z.intersection(z.lazy(() => BaseModelSchema), z.lazy(() => ExtendedModelSchema))');
    });
